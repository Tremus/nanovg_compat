...
void d3dPresent(void) { D3D_API_2(pSwapChain, Present, 0, 0); }
...
static HRESULT D3D__initializeDX11(HWND hwnd, unsigned int width,
                                   unsigned int height)
{
    HRESULT hr = S_OK;
    ...
    for (driver = 0; driver < ARRAYSIZE(driverAttempts); driver++)
    {
        hr = D3D11CreateDevice(NULL, driverAttempts[driver], NULL, deviceFlags,
                               ...
                               D3D11_SDK_VERSION, &pDevice, NULL,
                               &pDeviceContext);
    ...
    if (SUCCEEDED(hr))
    {
        hr = D3D_API_2(pDevice, QueryInterface, &IID_IDXGIDevice,
                       (void**)&pDXGIDevice);
        if (FAILED(hr))
        {
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = D3D_API_3(pDXGIFactory, CreateSwapChain, (IUnknown*)pDevice,
                       &swapDesc, &pSwapChain);
...
long d3dnvgSetViewBounds(void* hwnd, unsigned int width, unsigned int height)
{
    ...
    hr = D3D_API_5(pSwapChain, ResizeBuffers, swapDesc.BufferCount, width,
                   height, swapDesc.BufferDesc.Format, 0);
    if (FAILED(hr))
    {
    ...
    hr = D3D_API_3(pSwapChain, GetBuffer, 0, &IID_ID3D11Texture2D,
    ...
    hr = D3D_API_3(pDevice, CreateRenderTargetView, pBackBufferResource,
                   &renderDesc, &pRenderTargetView);
    ...
    hr = D3D_API_3(pDevice, CreateDepthStencilView,
                   (ID3D11Resource*)pDepthStencil, &depthViewDesc,
                   &pDepthStencilView);
    if (FAILED(hr))
    {
...
{
    D3D_API_2(pDeviceContext, ClearRenderTargetView, D3D__currentlyBoundView,
              color.rgba);
    D3D_API_4(pDeviceContext, ClearDepthStencilView, pDepthStencilView,
              D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 0.0f, 0u);
}
...
void d3dnvgBindFramebuffer(D3DNVGframebuffer* fb)
{
    ...
    D3D_API_3(pDeviceContext, OMSetRenderTargets, 1, &D3D__currentlyBoundView,
              pDepthStencilView);
                                           ...
                                           int flags)
{
    ...
    hr = D3D_API_3(pDevice, CreateRenderTargetView, tex->tex, &renderDesc,
                   &fb->pRenderTargetView);
    if (FAILED(hr))
    {
